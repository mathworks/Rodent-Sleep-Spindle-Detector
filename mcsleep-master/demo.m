...
%
waitfor(msgbox({'Welcome to Dr. Kevin Holly''s Spindle Analzyer using Ankit Parekh''s McSleep spindle detection method!';' ';'Let''s get those spindles!';' ';'Please select the folder containing the mat files you wish to analyze. If you used brainstorm to break your signal into many epochs then select the folder within the brainstorm_db folder that contains the epoch files.'},'Spindle Analyzer v1.0'))


%Select folder containing the data that you wish to analyze
default_dir = uigetdir('C:\Users\Chezinu\Documents\MATLAB\brainstorm_db');
files = dir(fullfile(default_dir,'data*.mat'));
epochs = length(files);
